...
app.post("/upload", upload.array("product", 6), (req, res) => {
  let imageUrls = req.files.map((file) => {
    return `https://shopnjs.onrender.com:4000/images/${file.filename}`;
  });
...
app.post("/website", productUpload.array("website", 6), (req, res) => {
  let imageUrls = req.files.map(
    (file) => `https://shopnjs.onrender.com:4000/images/${file.filename}`
  );
...
app.post("/uploadeds", supload.single("image"), (req, res) => {
  ...
  const imageUrl = `https://shopnjs.onrender.com:4000/images/${req.file.filename}`;
  const modelUrl = `https://shopnjs.onrender.com:4000/models/${req.file.filename}.glb`;
...
//   const screenshotUrl = `https://shopnjs.onrender.com:4000/website_screenshots/${req.file.filename}`;
